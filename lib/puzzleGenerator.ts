...
export function generatePuzzle(round: number): Puzzle {
  ...
  if (round <= 5) {
    ...
    selectedItems = [...easyItems, ...mediumItems].sort(() => Math.random() - 0.5);
  } else if (round <= 15) {
    ...
    selectedItems = [...mediumItems, ...hardItems].sort(() => Math.random() - 0.5);
  } else {
 ...
 */
export function checkAnswerWithAlternatives(guess: string, items: EmojiItem[]): boolean {
